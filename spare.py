...
# change selected color
    ...
    def del_record():
        ...
        my_tree.delete(x)
        cursor.execute("""delete from rental where reg_no = (select reg_no from car_details where car_id = (select car_id from car where agency_id=:a_id))""",
                       {
                           'a_id': e_id.get(),
                       })
        cursor.execute("""delete from car_details where car_id = (select car_id from car where agency_id=:a_id)""",
                       {
                           'a_id': e_id.get(),
                       })
