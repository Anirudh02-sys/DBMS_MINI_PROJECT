    ...
    for record in data:
        if count % 2 == 0:
            my_tree.insert(parent='', index='end', iid=count,
                           text="", values=(record[0], record[1]), tags=('evenrow',))
        else:
            my_tree.insert(parent='', index='end', iid=count,
                           text="", values=(record[0], record[1]), tags=('oddrow',))
        count += 1
...
def lookup():
    ...
    search_frame = LabelFrame(search, text="Agency ID")
...
def update_record():
    ...
    cursor.execute("""update agency set agency_name=:a_name where agency_name=:o_name """,
                   {
                       'o_name': o_name,
                       'a_name': e_name.get()
                   })
...
update_rec = Button(button_frame, text="Save record", command=update_record)
